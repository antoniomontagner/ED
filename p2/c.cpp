//!  Copyright [2020] <João Vitor Maia>
...
class BinaryTree {
 ...
 private:
    struct Node {
        ...
        int height() const {
            int left_size = left_ ? left_->height() : -1;
            int right_size = right_ ? right_->height() : -1;
            return std::max(left_size, right_size) + 1;
        }

        int leaves() const {
            int left_size = left_ ? left_->leaves() : 0;
            int right_size = right_ ? right_->leaves() : 0;

            int leaves = left_size + right_size;
        ...
        void filter(ArrayList<T>& v, int n_child) {
            int childs = 0;
            
            if (left_ != nullptr) {
                childs++;
                left_->filter(v, n_child);
            }

            if (right_ != nullptr) {
                childs++;
                right_->filter(v, n_child);
            }
    ...
    Node* root_{nullptr};
    std::size_t size_{0};
    void balance_tree(ArrayList<T> &array, int start, int end, BinaryTree<T> &tree);
};
...
//!     valor da árvore:
template<typename T>
ArrayList<T> BinaryTree<T>::    () {
    ArrayList<T> L(2);

    Node* min = root_->minimum();
    Node* max = root_->maximum();
...
BinaryTree<T> BinaryTree<T>::clone() {
    BinaryTree<T> C;
    ArrayList<T> preordered = pre_order();

    for (auto i = 0u; i < preordered.size(); i++)
        C.insert(preordered[i]);
...
BinaryTree<T> BinaryTree<T>::balance() {
    BinaryTree<T> B;
    ArrayList<T> inordered = in_order();
    balance_tree(inordered, 0, size()-1, B);
    return B;
}

template<typename T>
void BinaryTree<T>::balance_tree(ArrayList<T> &array, int start, int end, BinaryTree<T> &tree) {
    int i = (start + end) / 2;
    if (start <= end) {
        tree.insert(array.at(i));
        balance_tree(array, start, i-1, tree);
        balance_tree(array, i+1, end, tree);
    }
}
